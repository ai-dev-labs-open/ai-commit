...
describe('runCli', () => {
    it('prints plain text output for a normal staged diff', async () => {
        ...
        const exitCode = await runCli(['node', 'ai-commit'], {
    ...
    it('prints JSON output when requested', async () => {
        const stdout = new MemoryWriter()
        const exitCode = await runCli(['node', 'ai-commit', '--json'], {
            env: { ANTHROPIC_API_KEY: 'test-key' },
            stdout,
    ...
    it('fails when not inside a Git repository', async () => {
        ...
        const exitCode = await runCli(['node', 'ai-commit'], {
            ...
            gitClient: createGitClient(
                new CliError('Not inside a Git repository.', 'NOT_GIT_REPO')
            ),
            providerFactory: createProvider({
                rawText: '',
                subject: 'unused',
    ...
    it('fails when no staged changes exist', async () => {
        ...
        const exitCode = await runCli(['node', 'ai-commit'], {
            ...
            gitClient: createGitClient(
                new CliError(
                    'No staged changes found. Stage files before running ai-commit.',
                    'NO_STAGED_CHANGES'
                )
            ),
            providerFactory: createProvider({
    ...
    it('passes a truncated diff to the provider for oversized changes', async () => {
        const oversizedDiff = Array.from(
            { length: 4000 },
            (_, index) => `+line ${index} with additional content`
        ).join('\n')
    ...
    it('runs git commit when --commit flag is passed', async () => {
        ...
        const exitCode = await runCli(['node', 'ai-commit', '--commit'], {
            env: { ANTHROPIC_API_KEY: 'test-key' },
            stdout,
            stderr: new MemoryWriter(),
            gitClient: createGitClient(
